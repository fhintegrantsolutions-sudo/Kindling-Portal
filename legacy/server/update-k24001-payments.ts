...
// K24001 first payment was September 25, 2024
// Current date is February 9, 2026
// Payments are on the 25th of each month
// Payments made: September 2024 through January 2026 = 17 payments
...
async function updateK24001Payments() {
  console.log("Updating K24001 payment history...\n");

  // Find K24001 note
  const notesSnapshot = await db.collection("notes").where("noteId", "==", "K24001").get();
  if (notesSnapshot.empty) {
    console.error("Note K24001 not found!");
    return;
  }
  const noteDoc = notesSnapshot.docs[0];
  const noteId = noteDoc.id;
  const noteData = noteDoc.data();
  console.log(`Found K24001 note with ID: ${noteId}\n`);

  // Get all participations for K24001
  ...
  // First payment date: September 25, 2024
  ...
  for (const participationDoc of participationsSnapshot.docs) {
    ...
    const annualRate = parseFloat(noteData.rate) / 100; // 7.5%
    const termMonths = 60;
    ...
    for (let i = 0; i < paymentDates.length; i++) {
      ...
      totalPrincipalPaid += scheduleEntry.principal;
      totalInterestPaid += scheduleEntry.interest;
    }
  ...
  console.log(`Total paid: $${(totalPrincipalPaid + totalInterestPaid).toFixed(2)}`);
  console.log("========================================");
}

updateK24001Payments()
  .then(() => {
    console.log("\nDone!");
    process.exit(0);
  })
