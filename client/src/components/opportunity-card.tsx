...
export function OpportunityCard({ opportunity }: OpportunityCardProps) {
  ...
  const handleRegister = () => {
    toast({
      ...
      description: `You have requested to participate in ${opportunity.noteId}. A representative will contact you shortly.`,
    });
  };
  ...
  return (
    <Card className="flex flex-col h-full border-border/60 shadow-sm hover:shadow-lg transition-all duration-300" data-testid={`card-opportunity-${opportunity.id}`}>
      ...
      <CardFooter className="pt-2">
        <Button onClick={handleRegister} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium h-11 text-base shadow-md hover:shadow-lg transition-all" data-testid={`button-register-${opportunity.id}`}>
          Register Interest
        </Button>
      </CardFooter>
    </Card>
